...
from aiaccel.optimizer._grid_point_generator import (CategoricalGridCondition, FloatGridCondition, GridCondition,
                                                     GridConditionCollection, GridPointGenerator, GridValueType,
                                                     IntGridCondition, NumericGridCondition, NumericType,
                                                     OrdinalGridCondition, _cast_start_to_integer,
                                                     _cast_stop_to_integer, _create_grid_condition,
...
class TestGridConditionCollection:
    ...
    def test_get_grid_conditions_with_empty_choices(self, monkeypatch: pytest.MonkeyPatch) -> None:
        conditions = [
            FloatGridCondition(
                Parameter({"name": "x1", "type": "uniform_float", "lower": 0.0, "upper": 1.0, "log": False})
            ),
            CategoricalGridCondition(Parameter({"name": "x0", "type": "categorical", "choices": [0, 1, 2]})),
        ]
        with monkeypatch.context() as m:
            m.setattr(self.grid_condition_collection, "_conditions", conditions)
