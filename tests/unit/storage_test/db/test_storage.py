...
def test_current_max_trial_number():
    ...
    for i in range(len(states)):
        storage.state.set_any_trial_state(trial_id=i, state=states[i])
...
def test_get_hp_dict():
    ...
    exp = {
        ...
        "parameters": [{"name": param_name, "type": param_type, "value": param_value}],
        "result": objective,
...
def test_get_best_trial():
    storage = get_storage()
    trial_ids = [0, 1, 2, 3, 4]
    objectives = [[0.00], [0.01], [-1], [1], [np.float64(0.3)]]
    ...
    storage.result.set_any_trial_objective(trial_id=2, objective=[None])
